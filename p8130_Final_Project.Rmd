...
## Method
```{r}
summary(lm(sal9495 ~ rank, data = lawsuit))
```
...
```{r}
final_main_model = lm(sal9495 ~ gender + dept + exper + clin + cert , 
                      data = lawsuit)
summary(final_main_model)
```
...
```{r}
fit_inter1 = lm(sal9495 ~ gender * dept +  exper  + clin + cert, data = lawsuit)
summary(fit_inter1)
anova(final_main_model,fit_inter1)
```
...
```{r}
fit_inter2 = lm(sal9495 ~ gender * exper + dept + clin + cert, data = lawsuit)
summary(fit_inter2)
anova(final_main_model,fit_inter2)
```

genderMale:exper is significant since p value is less than 0.05.

partial F test p value < 0.05 
interaction between gender and exper is significant.

```{r}
fit_inter3 = lm(sal9495 ~ gender * clin +  exper + cert + dept, data = lawsuit)
summary(fit_inter3)
anova(final_main_model,fit_inter3)
```

???? wierd no main effect




```{r}
fit_inter4 = lm(sal9495 ~ gender * cert +  exper + clin + dept, data = lawsuit)
summary(fit_inter4)
anova(final_main_model,fit_inter4)
```
...
final_main_model_rank = 
  lm(sal9495 ~ gender + rank + exper + cert + clin + dept, data = lawsuit)
summary(final_main_model_rank)
```

grank interaction 
```{r}
fit_inter_rank = lm(sal9495 ~ gender * rank + exper + cert + clin + dept, data = lawsuit)
summary(fit_inter_rank)
anova(final_main_model_rank,fit_inter_rank)
par(mfrow = c(2, 2))
plot(fit_inter_rank)
```

```{r}
influence.measures(fit_inter_rank) %>% summary() %>% tidy()
### 184 outlier
newlawsuit = lawsuit[-184,]
model_grank_without184 = lm(sal9495 ~ gender * rank + exper + dept + clin + cert, data = newlawsuit)
par(mfrow = c(2, 2))
summary(model_grank_without184 )
plot(model_grank_without184)
```

ge interaction 
```{r}
fit_inter_ge = lm(sal9495 ~ gender*exper  + rank  + cert + clin + dept, data = lawsuit)
summary(fit_inter_ge)
anova(final_main_model_rank, fit_inter_ge)
par(mfrow = c(2, 2))
plot(fit_inter_ge)
```

```{r}
influence.measures(fit_inter_ge) %>% summary() %>% tidy()
### 184 outlier
newlawsuit = lawsuit[-184,]
model_ge_without184 = lm(sal9495 ~ gender * exper + rank + dept + clin + cert, data = newlawsuit)
...
```{r}
final = lm(sal9495 ~ gender + dept + exper + clin + cert+ gender * exper, 
                      data = lawsuit)
finalrank = lm(sal9495 ~ gender + dept + exper + clin + cert+ gender * exper+rank, 
                      data = lawsuit)
anova(final, finalrank)
summary(finalrank)
summary(final)
...
```{r}
final_model = lm(sal9495 ~ gender * exper + dept + clin + cert, data = lawsuit)
summary(final_model)
par(mfrow = c(2, 2))
plot(final_model)
...
```{r}
newlawsuit = lawsuit[-184,]
final_model_without184 = lm(sal9495 ~ gender * exper + dept + clin + cert, data = newlawsuit)
summary(final_model_without184 )
plot(final_model_without184)
```

Stratification: 
```{r}
male_data = newlawsuit %>% filter(gender == "Male") 
male_model = lm(sal9495 ~ exper + dept + clin + cert, data = male_data)
summary(male_model)
par(mfrow = c(2, 2))
plot(male_model)
```
...
```{r}
female_data = newlawsuit %>% filter(gender == "Female") 
female_model = lm(sal9495 ~ exper + dept + clin + cert, data = female_data)
