...
```{r}
fit4 = lm(sal9495 ~ gender + prate, data = lawsuit)
summary(fit4)
anova(reg1,fit4)
```
...
fit5 = lm(sal9495 ~ gender + exper, data = lawsuit)
summary(fit5)
anova(reg1,fit5)
...
```{r}
fit6 = lm(sal9495 ~ gender + rank, data = lawsuit)
summary(fit6)
anova(reg1,fit6)
```
...
```{r}
## add prate
fit_gdp = lm(sal9495 ~ gender + dept + prate, data = lawsuit)
summary(fit_gdp)
anova(reg2,fit_gdp)
```
...
```{r}
## add prate
fit_gdep = lm(sal9495 ~ gender + dept + exper + prate, data = lawsuit)
summary(fit_gdep)
anova(reg3,fit_gdep)
```
...
```{r}
## add cert
fit_gdeclin_prate = lm(sal9495 ~ gender + dept + exper + clin + prate, data = lawsuit)
summary(fit_gdeclin_prate)
anova(reg4,fit_gdeclin_prate)
```
...
p value of partial F test is more than 0.05, which shows that the model adding prate is not superior. 
