...
-----------------------------------------
**Compare with two interaction!!**

grank interaction 
```{r}
fit_inter_grank = lm(sal9495 ~ gender * rank + exper + cert + clin + dept, data = lawsuit)
summary(fit_inter_grank)
anova(final_main_model_rank,fit_inter_grank)
par(mfrow = c(2, 2))
plot(fit_inter_grank)
```
...
```{r}
fit_inter_gerank = lm(sal9495 ~ gender*exper  + gender*rank  + cert + clin + dept, data = lawsuit)
summary(fit_inter_gerank)

```


Compare gender* rank and gender* exper +gender *rank
```{r}
anova(fit_inter_grank,fit_inter_gerank)
```

Compare gender* exper and gender* exper +gender *rank
```{r}
anova(fit_inter_ge, fit_inter_gerank)
```
...
```{r}
AIC(fit_inter_ge) ## gender*exper
AIC(fit_inter_grank) ## gender*rank
```

**Results**

gender * exper helps in the model, gender * rank not. 
Choose interaction gender * exper.
...
```{r}
final_model = lm(sal9495 ~ gender * exper + dept + clin + cert + rank, data = lawsuit)
summary(final_model)
```
...
fit_rank = lm(sal9495 ~ gender*rank+dept+clin+cert+exper+gender*exper, data = lawsuit)
summary(fit_rank)
anova(final_model,fit_rank)
```
## on rank
```{r}
# Associate
...
exper = lawsuit %>% 
  mutate(experience = ifelse(exper<5, "0",
                             ifelse(exper<10&exper>=5, "1",
                                    ifelse(exper<14&exper>=10,"2","3"))),
...
experience0 = exper %>% 
  filter(experience == "0") %>% 
  lm(sal9495 ~ gender+dept+clin+cert+rank, data = .)
summary(experience0)
...
Gender does not play a huge role in different levels of experience.

## on gender

```{r}
male_data = newlawsuit %>% filter(gender == "Male") 
male_model = lm(sal9495 ~ exper + dept + clin + cert + rank, data = male_data)
summary(male_model)
par(mfrow = c(2, 2))
plot(male_model)
...
female_model = lm(sal9495 ~ exper + dept + clin + cert + rank, data = female_data)
